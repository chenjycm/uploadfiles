...
$.fn.Huploadify = function(opts){
	var itemTemp = '<div id="${fileID}" class="uploadify-queue-item"><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div><span class="up_filename">${fileName}</span><span class="uploadbtn">上传</span><span class="delfilebtn glyphicon glyphicon-remove" aria-hidden="true"></span></div>';
	var defaults = {
		  ...
		  onSelect: function(files){
				for(var i=0,len=files.length;i<len;i++){
					...
					if(option.showUploadedSize){
						var num = '</br><span class="progressnum"><span class="uploadedsize">0KB</span>/<span class="totalsize">${fileSize}</span></span>'.replace(/\${fileSize}/g,formatFileSize(file.size));
						$html.find('.delfilebtn').after(num);
					}
